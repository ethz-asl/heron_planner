...
class UgvMove(Move):
    ...
    def __init__(
        ...
        #TODO setup service call here
        # srv_ = rospy.Service("/command_manager/command")
    ...
    def compute_twist(self) -> Twist:
        ...
        rospy.loginfo(
            f"MoveX: Sending velocity {twist_msg.linear.x:.2f} [m/s] to x"
        )
        rospy.loginfo(
            f"MoveY: Sending velocity {twist_msg.linear.y:.2f} [m/s] to y"
        )
        rospy.loginfo(
            f"MoveYaw: Sending velocity {twist_msg.angular.z:.2f} [rad/s] to yaw"
        )
...
class UgvMoveX(Move):
    ...
    def compute_twist_x(self) -> Twist:
        ...
        twist_msg.linear.x = np.clip(
            vel, -self.max_lin_vel / 2, self.max_lin_vel / 2
        )
    ...
    def at_goal_x(self) -> bool:
        ...
        rospy.logwarn(
            f"target_x: {target_x:.2f}, delta_x_local: {delta_x_local:.2f}"
        )
...
class UgvMoveY(Move):
    ...
    def compute_twist_y(self) -> Twist:
        ...
        rospy.loginfo(f"MoveY: Sending velocity {twist_msg.linear.y:.2f} [m/s]")
        return twist_msg
...
class UgvTurnYaw(Move):
    ...
    def compute_twist_yaw(self) -> Twist:
        ...
        rospy.logwarn(
            f"[target, cur]: [{self.target_yaw:.2f}, {current_yaw:.2f}]"
        )
        ...
        twist_msg.linear.y = 0.0
        twist_msg.angular.z = np.clip(
            vel, -self.max_ang_vel / 2, self.max_ang_vel / 2
        )
...
class UgvComponents:
    ...
    def init_publishers(self):
        self.hatch_up_pub = rospy.Publisher(
            "/hatch_up", Bool, queue_size=10, latch=True
        )
        self.roller_up_pub = rospy.Publisher(
            "/roller_up", Bool, queue_size=10, latch=True
        )
    ...
    def init_srvs(self):
        ...
        self.close_hatch_srv = rospy.get_param(
            "~close_hatch_srv", "/close_hatch"
        )
        self.raise_roller_srv = rospy.get_param(
            "~raise_roller_srv", "/raise_roller"
        )
        self.lower_roller_srv = rospy.get_param(
            "~lower_roller_srv", "/lower_roller"
        )
    ...
    def init_gazebo_models(self):
        ...
        if model_mesh_path not in gazebo_model_path:
            new_gazebo_model_path = (
                f"{gazebo_model_path}:{model_mesh_path}"
                if gazebo_model_path
                else model_mesh_path
            )
    ...
    def open_hatch(self) -> bool:
        ...
        if not gazebo_utils.model_exists(self.arr_name):
            gazebo_utils.spawn_model(
                self.arr_name, self.arr_path, self.open_pose
            )
        else:
    ...
    def close_hatch(self):
        ...
        if not gazebo_utils.model_exists(self.arr_name):
            gazebo_utils.spawn_model(
                self.arr_name, self.arr_path, self.close_pose
            )
        else:
...
class UgvAtPose(AtPose):
    ...
    def at_pose(
        self, goal_pose: Pose, tol: float = 0.01, ground_truth: bool = True
    ) -> bool:
        ...
        disp_xy = round(np.linalg.norm(current_xy - goal_xy) - 0.5, 3)
        delta_yaw = round(
            utils.wrap_angle(
                np.arctan2(
        ...
        rospy.logwarn(
            f"Robot at dist {abs(disp_xy):.2}[m] and {delta_yaw:.2}[rad] from goal"
        )
